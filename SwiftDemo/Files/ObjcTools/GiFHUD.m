...
#define APPDELEGATE     UIApplication.sharedApplication.keyWindow
...
- (instancetype)init {
    if ((self = [super initWithFrame:CGRectMake(0, 0, Size, Size)])) {
        ...
        [self setAlpha:0];
        [self setCenter:APPDELEGATE.center];
...
+ (void)showWithOverlay {
    [self dismiss:^{
        [APPDELEGATE addSubview:[[self instance] overlay]];
        [self show];
    }];
}
...
+ (void)show {
    [self dismiss:^{
        [APPDELEGATE bringSubviewToFront:[self instance]];
...
- (UIView *)overlay {
    ...
    if (!self.overlayView) {
        self.overlayView = [[UIView alloc] initWithFrame:APPDELEGATE.frame];
